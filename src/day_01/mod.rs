pub fn parts() {
    ...
    for line in include_str!("../../input/day_01.txt").split("\n") {
        ...
        let p1_val = format!("{}{}", p1_digits[0], p1_digits[p1_digits.len()-1]);
        let p2_val = format!("{}{}", p2_digits[0], p2_digits[p2_digits.len()-1]);
        p1 = p1 + p1_val.parse::<i32>().unwrap();
        p2 = p2 + p2_val.parse::<i32>().unwrap();
    }
    println!("{}", p1);
    println!("{}", p2);
}
